...
class MyQRCodeReader {
    ...
    func setupCamera( view:UIView, borderWidth:Int = 1, borderColor:CGColor =  UIColor.red.cgColor ){
        ...
        /*let Switch:UISwitch = UISwitch(frame: CGRect(x: 50, y: 640 , width: 49, height: 31))
        Switch.isOn = false
        Switch.addTarget(self, action: #selector(onClick), for: UIControl.Event.valueChanged)
        self.preview?.addSubview(Switch)
        
        let Message:UILabel = UILabel(frame: CGRect(x: 110, y: 640 , width: 200, height: 31))
        Message.text = "備品追加の場合はONに！"
        //Message.textColor = UIColor.red
        self.preview?.addSubview(Message)*/
    }
    /*
    @objc func onClick(sender: UISwitch) -> Bool{
    ...
    //読み取り範囲の指定
    public func readRange( frame:CGRect = CGRect(x: 0.2, y: 0.05, width: 0.6, height: 0.3) ){
